...
- **LLM 专家审核** — 场景输入/输出现在可以由 AI 专家（销售策略、客服、数据提取等）进行深度审核，支持降级到关键词模式
- **15 个专家审核评测用例** — 覆盖材料审核、输出审核、幻觉检测三个维度，CI 已集成
- **E2E 测试覆盖** — 新增 Task 和 Doc 的端到端测试（26 个测试用例），确保 lark-cli 集成正常

### 修复

- **prompt_file 路径规范化** — 专家提示文件路径格式统一，避免加载失败
- **LLM 调用异步修复** — 修复 async 阻塞事件循环的问题
- **RegistryCache 失效逻辑** — 监控 agent-registry.yaml 文件变动而非目录，保证编辑后正确刷新
