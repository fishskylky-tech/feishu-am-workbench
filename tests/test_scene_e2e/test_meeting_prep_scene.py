"""Meeting prep scene E2E tests.

Calls run_meeting_prep_scene() against the live Feishu test environment.
...
from runtime.scene_runtime import SceneRequest, SceneResult, run_meeting_prep_scene
...
def make_meeting_prep_request(repo_root, customer_query, topic_text="会前准备测试", meeting_type="Q2目标对齐"):
    return SceneRequest(
...
def test_happy_path_returns_result(repo_root, test_customer_query):
    """Meeting prep scene returns a SceneResult."""
    request = make_meeting_prep_request(
        repo_root=repo_root,
        customer_query=test_customer_query,
    )
    result = run_meeting_prep_scene(request)
