"""Proposal scene E2E tests.

Calls run_proposal_scene() against the live Feishu test environment.
...
from runtime.scene_runtime import SceneRequest, SceneResult, run_proposal_scene
...
def make_proposal_request(repo_root, customer_query, proposal_type="proposal", topic_text="提案测试"):
    return SceneRequest(
        scene_name="proposal",
        repo_root=repo_root,
...
def test_happy_path_returns_result(repo_root, test_customer_query):
    """Proposal scene returns a SceneResult."""
    request = make_proposal_request(
        repo_root=repo_root,
        customer_query=test_customer_query,
    )
    result = run_proposal_scene(request)
